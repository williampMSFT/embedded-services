...
// TODO I think for non-passthrough cases it'd be good for ergonomics to have some sort of macro that can take a collection of
//      annotated types and generate a HID report descriptor for a device that contains all of those types in some fashion.
//      Not a ship-blocker, but would be nice to have so we don't need to run an external tool like waratah, generate a descriptor
//      from that, then generate a bunch of corresponding repr(C) structs that can get out of sync with the descriptor
...
impl<'a> HidReportDescriptor<'a> {
    ...
    ///
    /// Returns [`HidDescriptorError`] if `bytes` is malformed
    ///
    pub fn new(bytes: &'a [u8]) -> Result<Self, HidDescriptorError> {
        ...
        // Bits accumulated for the report currently being described, and the largest report of each
        // type seen so far. `current` is folded into `max` and cleared whenever the report ID changes.
        ...
        let mut stack = heapless::Vec::<GlobalItemState, PUSH_POP_STACK_DEPTH>::new();

        for item in DescriptorItems::new(bytes) {
            let item = item?;
            if item.header.is_long_item() {
                // Long items (e.g. vendor-defined) don't affect report sizing.
                continue;
            }
            ...
            match item.header.item_type() {
                HidItemType::Global => {
                    ...
                    if tag == GlobalItemTag::ReportSize as u8 {
                        ...
                        // A Pop that restores a different report ID likewise ends the current report.
...
/// The maximum payload size, in bytes, of each report type declared by a HID report descriptor, as
/// computed by [`HidReportDescriptor::max_report_sizes`]. Sizes exclude the report ID byte.
...
/// Maximum `Push` global-item nesting depth supported by [`HidReportDescriptor::max_report_sizes`].
const PUSH_POP_STACK_DEPTH: usize = 32;
...
///
/// Tags are is only meaningful relative to the item type; these are the Global-type tags.
#[derive(Clone, Copy, Debug, PartialEq, Eq, num_enum::IntoPrimitive, num_enum::TryFromPrimitive)]
#[repr(u8)]
enum GlobalItemTag {
    ReportSize = 0b0111,
...
impl<const REPORT_COUNT: usize, const DEVICE_COUNT: usize> ReportIdMap<REPORT_COUNT, DEVICE_COUNT> {
    ...
    /// storage, and captures the resulting report-ID remapping in a fresh `ReportIdMap`.
    ///
    ...
    pub fn combine<'buf>(
        ...
        for (input_index, input) in inputs.iter().enumerate() {
            ...
            // An implicit descriptor (no report ID item) still needs a unique report ID, so
            // we have to insert an explicit report ID item in the output descriptor.
            if input.report_ids_implicit() {
...
/// Only intended for consumption from within this module but must be public because it's used in
/// the [`impl_hid_aggregate_device!`](crate::impl_hid_aggregate_device) macro
...
/// - On an error from any child, returns [`HidError::TriggerReset`], which triggers a device-initiated
///   reset of the *entire* aggregate (and therefore all children).
///
...
macro_rules! impl_hid_aggregate_device {
    ...
    (@emit $vis:vis $out:ident ; $($idx:tt : $dev:ty),+ ) => {
        ...
        $vis struct [< $out Resources >] {
            // Backing storage the combined report descriptor is written into and borrowed from.
            descriptor: $crate::_macro_internal::heapless::Vec<
                u8,
                {
        ...
        impl [< $out Resources >] {
            /// Create empty storage for the aggregate's combined report descriptor.
            $vis const fn new() -> Self {
                Self { descriptor: $crate::_macro_internal::heapless::Vec::new() }
            }
        }
        ...
        impl<'a> $out<'a> {
            ...
            $vis fn new(
                ...
                resources.descriptor.clear();
                let _ = resources.descriptor.resize(resources.descriptor.capacity(), 0u8);
...
mod tests {
    ...
    // Report ID. The report ID value lives at index 7.
    fn explicit_descriptor(report_id: u8) -> [u8; 17] {
        [
    ...
    /// the tests that only care about the combined bytes, not the remapping. `REPORTS` (the `remaps`
    /// capacity) is given via turbofish; the device count is inferred from the `inputs` array.
    ...
    fn implicit_descriptor_gets_report_id_inserted() {
        ...
        // Each input is wrapped in Push/Pop; the inserted Report ID item sits just inside the Push.
        assert_eq!(combined.as_bytes()[0], PUSH_HEADER.byte());
        assert_eq!(combined.as_bytes()[1], REPORT_ID_HEADER_SIZE1.byte());
        assert_eq!(combined.as_bytes()[2], 1);
    }
    ...
    fn remaps_report_conflict_resolution() {
        // Two sub-devices that both natively use report ID 1 (a genuine conflict).
        let a = explicit_descriptor(1);
        let b = explicit_descriptor(1);
        let mut out = [0u8; 64];
        // REPORT_COUNT capacity 8; DEVICE_COUNT (2) is inferred from the input array length.
    ...
    // Two static explicit descriptors, both natively using report ID 1 (a deliberate conflict).
    static DESC_ID1_A: [u8; 17] = explicit_descriptor_const(1);
    static DESC_ID1_B: [u8; 17] = explicit_descriptor_const(1);

    // A `const fn` mirror of `explicit_descriptor`, so we can build `'static` descriptor bytes for
    // the mock devices used to exercise the aggregate macro.
    const fn explicit_descriptor_const(report_id: u8) -> [u8; 17] {
        [
            0x05, 0x01, 0x09, 0x02, 0xA1, 0x01, 0x85, report_id, 0x09, 0x30, 0x75, 0x08, 0x95, 0x01, 0x81, 0x02, 0xC0,
        ]
    }
    ...
    impl MockDev {
        fn new(bytes: &'static [u8], tag: u8, native_id: u8) -> Self {
            Self {
                descriptor: HidReportDescriptor::new(bytes).unwrap(),
                tag,
                native_id,
                has_input: false,
                resets: 0,
    ...
    fn aggregate_combines_descriptor_and_report_sizes() {
        ...
        let agg = TestAggregate::new(
            &mut resources,
            MockDev::new(&DESC_ID1_A, 0, 1),
            MockDev::new(&DESC_ID1_B, 1, 1),
        )
        .unwrap();
